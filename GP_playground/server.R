...
shinyServer(function(input, output) {
  ...
  observeEvent(input$plotModel_click, {
    if(input$addPts2){
      XY$X <- c(XY$X,input$plotModel_click$x)
      XY$Y <- c(XY$Y,input$plotModel_click$y)
    }else{
      nearPt <- nearPoints(XY$X,XY$Y,input$plotModel_click$x,input$plotModel_click$y)
      if(length(nearPt)){
        XY$X <- XY$X[-nearPt]
        XY$Y <- XY$Y[-nearPt]
      }
  ...
  output$model <- renderPlot({
    ...
    if(length(XY$X)==0){
      ...
      mean <- mean+KxX %*% KXX_1 %*% (Y-Ymean)
      var <- Kxx - KxX %*% KXX_1 %*% t(KxX)
      var <- diag(var)

    }
    par(mar=c(4.5,5.1,1.5,1.5),cex.axis=1.2,cex.lab=1.5)
    plotGPR(x,mean,var)
    points(XY$X,XY$Y,pch=4, cex=1,lwd=3)
  })
  ...
  output$samples <- renderPlot({
    ...
    matplot(x,Z,type='l',lty=1,xlab="x",ylab="",main="",col=mycols)
    points(XY$X,XY$Y,pch=4, cex=1,lwd=3)
  })
